...
- 各ユーザーIDに対して以下の順序で処理を行い、結果をスプレッドシートの「出力結果」シートに記録します。
  ...
  4. 各コメント投稿者のプロフィールページにアクセスし、非公開判定、フォロワー数、フォロー数を一括取得
  5. 取得した全データ（コメントユーザーID、プロフィールURL、アカウント状態、フォロワー数、フォロー数等）をスプレッドシートに一括追記
...
- **データ取得ロジック**:
  ...
  - `get_profile_info`: 指定されたユーザーIDのプロフィールにアクセスし、アカウント状態（公開/非公開）、フォロワー数、フォロー数を一括で取得します。
...
- **出力結果シート**: [取得日時, ターゲットID, 投稿URL, コメントしたユーザーのID, ユーザーURL, アカウント状態, フォロワー数, フォロー数, ステータス] の形式で追記します。
