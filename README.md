...
- 各ユーザーIDに対して以下の順序で処理を行い、結果をスプレッドシートの「出力結果」シートに記録します。
  ...
  4. 各コメント投稿者のプロフィールページにアクセスし、非公開判定、フォロワー数、フォロー数、プロフィール文（Bio）を一括取得
  5. 取得した全データ（コメントユーザーID、コメント内容、プロフィールURL、アカウント状態、フォロワー数等）をスプレッドシートに一括追記
...
- **データ取得ロジック**:
  ...
  - `get_commenting_users`: 各コメントのコンテナ要素（`ul.x1qjc9v5 > li` 等）を起点としたループ処理により、ユーザーIDとコメント内容をペアで抽出します。同一ユーザーの複数コメントは ` / ` で結合して重複を排除します。戻り値は辞書のリスト（`[{"user_id": "...", "comment_text": "..."}]`）です。
  - `get_profile_info`: 指定されたユーザーIDのプロフィールにアクセスし、アカウント状態（公開/非公開）、フォロワー数、フォロー数、プロフィール文（Bio）を一括で取得します。
  - `_convert_stat_to_int`: "1.2k" や "1,500" といったInstagram特有の統計テキストを数値（int）に変換します。
  - `get_comment_count`: 投稿ページ内の特定の要素から、正規表現を用いてコメント数を抽出します。
- **デバッグ機能**:
