...
- 各ユーザーIDに対して以下の順序で処理を行い、結果をスプレッドシートの「出力結果」シートに記録します。
  ...
  4. 取得した全データ（コメントユーザーID、プロフィールURL等）をスプレッドシートに一括追記
...
- **データ取得ロジック**:
  ...
  - `get_commenting_users`: 指定された投稿URLから、コメントしているユーザーのIDリストを抽出します（投稿者本人は除外）。「コメント...件をすべて見る」ボタン（「いいね」ボタンと重複するため2番目の要素を指定）が存在する場合は自動的にクリックして全コメントを展開してから抽出を行います。
  - `get_comment_count`: 投稿ページ内の特定の要素（`span[role="button"].x1ypdohk` 等）から、2番目の要素（`nth(1)`）を明示的に指定して正規表現を用いてコメント数を抽出します。これにより「いいね」ボタンとの重複を回避しています。要素数が足りない場合や取得エラー時は、安全に `0` を返す例外処理を実装しています。
- **デバッグ機能**:
...
- **出力結果シート**: [取得日時, ターゲットID, 投稿URL, コメントしたユーザーのID, ユーザーURL, ステータス] の形式で追記します。
