...
- 各ユーザーIDに対して以下の順序で処理を行い、結果をスプレッドシートの「出力結果」シートに記録します。
  ...
  4. 各コメント投稿者のプロフィールページにアクセスし、非公開アカウント（鍵垢）かどうかを判定
  5. 取得した全データ（コメントユーザーID、プロフィールURL、アカウント状態等）をスプレッドシートに一括追記
...
- **データ取得ロジック**:
  ...
  - `is_private_account`: 指定されたユーザーIDのプロフィールにアクセスし、ページ内のテキスト（「このアカウントは非公開です」等）を元にアカウントの状態を判定します。Bot検知回避のため、アクセス前後に2〜5秒のランダムな待機時間を設けています。
...
- **出力結果シート**: [取得日時, ターゲットID, 投稿URL, コメントしたユーザーのID, ユーザーURL, アカウント状態, ステータス] の形式で追記します。
