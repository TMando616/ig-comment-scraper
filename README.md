...
- 各ユーザーIDに対して以下の順序で処理を行い、結果をスプレッドシートの「出力結果」シートに記録します。
  ...
  5. 取得した全データ（コメントユーザーID、プロフィールURL、アカウント状態、フォロワー数、フォロー数、プロフィール文等）をスプレッドシートに一括追記
...
- **データ取得ロジック**:
  ...
  - `_convert_stat_to_int`: "1.2k" や "1,500" といったInstagram特有の統計テキストを数値（int）に変換します。'k'（1,000倍）や 'M'（1,000,000倍）の単位変換にも対応しています。
...
- **出力結果シート**: [取得日時, ターゲットID, 投稿URL, コメントしたユーザーのID, ユーザーURL, アカウント状態, フォロワー数, フォロー数, プロフィール文, ステータス] の形式で追記します。
