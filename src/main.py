...
def main():
    ...
    # コマンドライン引数から取得投稿数を受け取る
    target_post_count = Config.DEFAULT_TARGET_POST_COUNT
    if len(sys.argv) > 1:
        try:
            target_post_count = int(sys.argv[1])
        except ValueError:
            logger.warning(f"引数 '{sys.argv[1]}' が数値ではありません。デフォルトの {target_post_count} を使用します。")

    logger.info(f"=== Instagram Comment Scraper 起動 (取得件数: {target_post_count}) ===")
    logger.info(f"設定: ターゲットシート='{Config.TARGET_SHEET_NAME}', 出力結果シート='{Config.RESULT_SHEET_NAME}'")
    ...
    try:
        ...
        logger.exception("スプレッドシートマネージャーの初期化に失敗しました")
        return
    
    user_ids = ss_manager.get_target_user_ids()
    if not user_ids:
        logger.warning("処理対象のユーザーIDが見つかりませんでした。")
        return

    logger.info(f"{len(user_ids)} 名のユーザーを処理します。")
    ...
    with sync_playwright() as playwright:
        try:
            if not scraper.login(playwright):
                logger.error("ログインに失敗したため、処理を中断します。")
                return

            for user_id in user_ids:
                logger.info(f"--- ユーザー処理開始: {user_id} ---")
                
                # 1. 投稿URLを取得
                logger.info(f"最新の {target_post_count} 件の投稿を取得します...")
                ...
                if not post_urls:
                    ...
                    logger.info(f"プロフィール処理結果: {profile_status}")
                    continue

                logger.info(f"取得した {len(post_urls)} 件の投稿を巡回します...")
                ...
                if all_rows:
                    if ss_manager.append_results(all_rows):
                        logger.info(f"結果を {len(all_rows)} 件記録しました。")
                    else:
                        logger.error("スプレッドシートへの記録に失敗しました。")
                else:
                    logger.info("記録するデータがありませんでした。")

        except Exception:
            logger.exception("メインループ内で予期せぬエラーが発生しました")
        finally:
