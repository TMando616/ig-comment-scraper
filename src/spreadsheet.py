...
class SpreadsheetManager:
    ...
    def append_results(self, rows: List[List[Any]]) -> bool:
        ...
        rowのフォーマット: [取得日時, ターゲットID, 投稿URL, コメントユーザーID, ユーザーURL, ステータス]
        """
        try:
            if not rows:
                return True
    ...
    def append_result(self, user_id: str, post_url: str, commenter_id: Any, commenter_url: str, status: str) -> bool:
        """
        「出力結果」シートに1行の結果を追記する
        フォーマット: [取得日時, ターゲットユーザーID, 投稿URL, コメントユーザーID, ユーザーURL, ステータス]
        """
        try:
            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            row = [now, user_id, post_url, commenter_id, commenter_url, status]
