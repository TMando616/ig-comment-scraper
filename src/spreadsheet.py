...
class SpreadsheetManager:
    ...
    def append_results(self, rows: List[List[Any]]) -> bool:
        ...
        rowのフォーマット: [取得日時, ターゲットID, 投稿URL, コメントユーザーID, コメント内容, ユーザーURL, アカウント状態, フォロワー数, フォロー数, プロフィール文, ステータス]
        """
        try:
            if not rows:
                return True
    ...
    def append_result(self, user_id: str, post_url: str, commenter_id: Any, comment_text: str, commenter_url: str, account_status: str, followers: int, following: int, bio: str, status: str) -> bool:
        """
        「出力結果」シートに1行の結果を追記する
        フォーマット: [取得日時, ターゲットユーザーID, 投稿URL, コメントユーザーID, コメント内容, ユーザーURL, アカウント状態, フォロワー数, フォロー数, プロフィール文, ステータス]
        """
        try:
